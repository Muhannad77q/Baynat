...
self.addEventListener("message", (event) => {
  const { token, difficultyBits } = event.data || {};
  if (
    typeof token !== "string" ||
    ...
    self.postMessage({ error: "invalid-challenge" });
    return;
  }

  for (let counter = 0; counter < Number.MAX_SAFE_INTEGER; counter += 1) {
    if (hasLeadingZeroBits(sha256(`${token}.${counter}`), difficultyBits)) {
      self.postMessage({ counter });
      return;
    }
  }
  self.postMessage({ error: "challenge-exhausted" });
});
